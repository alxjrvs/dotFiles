...
The standing safety net under auto mode is **least privilege at the credential boundary**: the GitHub MCP and agent git use fine-grained PATs, and 1Password access is a **service account scoped to a single `claude-agent` vault** (1Password forbids a service account from reading Personal/Private — that one vault is the whole blast radius), so secrets resolve with no Touch ID prompt and no desktop-app dependency. See [Secrets](#secrets).
...
- **Agent git auth** — the agent authenticates git-over-HTTPS via the stock **`osxkeychain`** helper so it pushes with a least-privilege, fine-grained PAT instead of your broad `gh` OAuth token, headlessly (a keychain read, no biometric). `dot sync` resolves a fine-grained `Contents:read+write` PAT from the `claude-agent` vault and caches it in the login keychain; a Claude session only *reads* it. Wired *agent-only* via `GIT_CONFIG_*`, while your terminal git keeps the `gh` helper. Rotation = re-run `dot sync`. (SSH auth was rejected for the agent — 1Password's SSH agent is interactive-only.)
