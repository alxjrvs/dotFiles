...
- **Agent git auth** — the same pattern backs git-over-HTTPS: `gh/git-credential-op-claude` (a git credential helper) resolves a fine-grained `Contents:read+write` PAT from the `claude-agent` vault. It's wired *agent-only* (via Claude's `GIT_CONFIG_*` env), so a Claude session pushes with a least-privilege PAT instead of your broad `gh` OAuth token, while your own terminal git keeps the `gh` helper. (SSH auth was rejected for the agent — 1Password's SSH agent is interactive-only and can't run headless.)
