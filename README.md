...
[**botu**](https://github.com/alxjrvs/botu) — the small TypeScript dotfiles+workspace
...
| `botu apply --only="zsh fragments"` | Only the named section (e.g. brew/mise packages). |
...
| `botu code claude\|cmux` | Mirror `~/Code` into agent workspaces. `claude` symlinks every repo into one flat, `@`-taggable dir and opens `claude agents` there (no running agents); `cmux` opens one cmux workspace per repo. |
...
| 1Password signing-key item name | `botufile` / the `git-signing` setup (default `GitHubSSH`) |
...
| Statusline source repo | `hooks/claude_statusline.ts` (`repo=…` in the botufile) |
...
| Path | Purpose |
|------|---------|
| `botufile.toml` | The config botu reads — symlink table, packages, macOS defaults, inline steps, hooks |
| `botuinit.sh` | Fresh-machine bootstrap (install botu → init → apply); generated by `botu init` |
| `hooks/` | Imperative escape hatches: `op-agent.sh` (1Password-agent CLI), `git-signing.ts` (1Password commit signing), `claude_statusline.ts` |
