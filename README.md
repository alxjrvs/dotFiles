...
- `settings.json` — deliberately minimal (agent teams, statusline, auto mode, lightweight bash sandbox, quieter UI); the full list lives in `dot-claude/CLAUDE.md`
...
Two safety nets layer under auto mode. **Least privilege at the credential boundary**: the GitHub MCP and agent git use fine-grained PATs, and 1Password access is a **service account scoped to a single `claude-agent` vault** (1Password forbids a service account from reading Personal/Private — that one vault is the whole blast radius), so secrets resolve with no Touch ID prompt and no desktop-app dependency. And **OS containment** via a lightweight Seatbelt **sandbox** (`sandbox.enabled`, `allowUnsandboxedCommands:false`) — workdir-confined writes, no dynamic escape, and deliberately carve-out-free (no 1Password-specific config, because secrets resolve *outside* it). One consequence: `op` can't run in-box (the sandbox MITM-proxies TLS and `op` pins system trust), so MCP auth resolves outside the sandbox and agent git auth reads an `osxkeychain`-cached PAT. Repos needing system-wide writes or in-box `op` (this installer) opt out via a committed `.claude/settings.json`. See [Secrets](#secrets).
...
- **Agent git auth** — `op` can't run inside the bash sandbox (it pins system trust; the sandbox MITM-proxies TLS), so the agent authenticates git-over-HTTPS via the stock **`osxkeychain`** helper instead. `dot sync` (unsandboxed) resolves a fine-grained `Contents:read+write` PAT from the `claude-agent` vault and caches it in the login keychain; a sandboxed Claude session only *reads* it (keychain reads work in-box). Wired *agent-only* via `GIT_CONFIG_*`, so the agent pushes with a least-privilege PAT instead of your broad `gh` OAuth token, while your terminal git keeps the `gh` helper. Rotation = re-run `dot sync`. (SSH auth was rejected for the agent — 1Password's SSH agent is interactive-only.)
