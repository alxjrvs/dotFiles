...
- One source, two hosts: a Mac has everything; Linux is a container with no 1Password app, no
  ...
  `.chezmoi.os` alone. No by-hand step remains that `chezmoi apply` could converge, except
  removals and external bumps.
...
3. Open a PR. `lint` is the one required check; CI runs the same two tasks on macOS and Ubuntu.
...
- Every CLI and runtime is a mise tool (`home/dot_config/mise/config.toml`), on both OSes. The
  exceptions are the `1password-cli` and `gcloud-cli` casks, and `postgresql@17`, a service brew
  runs. A Mac's apps are Brewfile casks. chezmoi, mise and the Claude Code CLI come from their
...
- `chezmoi apply` never upgrades anything, and it uninstalls any Homebrew package the Brewfile
  does not name.
- Machine setup is `home/.chezmoiscripts/`, kept flat. Homebrew, sleep and provisioning run on
  every apply behind a guard. mise, the Brewfile and the Caps Lock plist re-run when their file
  changes, and the macOS defaults re-run when the script itself changes.
...
- One GitHub login: `gh auth login`. It is stored in the login keychain on a Mac, and in a
  plaintext `~/.config/gh/hosts.yml` in a container. Never `--insecure-storage` on a Mac.
