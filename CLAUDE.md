...
- **Hands-off agent (Claude, MCP, cron)** — a **service account** whose home is the `claude-agent` vault, additionally granted read-only on the shared `Gnar` vault for the single Ninety.io PAT the `gninety` MCP expects there (an explicit, owner-approved blast-radius tradeoff — see *MCP secrets*); no biometric, no desktop dependency.
...
- `op-agent provision` — idempotently ensures the `claude-agent` vault, a per-host service account with `read_items` on that vault, and its token in the macOS login keychain (`op-claude-agent`); also confirms the `Claude Git PAT` vault item exists (a fresh-machine setup signal — the PAT is resolved on demand, never cached). Run via `on apply op-agent provision`. Foreground-only first run (minting authorizes through the desktop app). **Does not grant the `Gnar` read access** the `gninety` MCP needs (see *MCP secrets*) — that's a manual owner action in the 1Password Console (SA `claude-agent-<LocalHostName>` → add `Gnar:read_items`), so a fresh machine must repeat it or gninety 401s.
