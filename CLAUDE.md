...
- **`worktree-freshness.sh`** — `SessionStart` (matcher `startup|resume`) + `SubagentStart`. Not a guard: it decides nothing and denies nothing, it *writes*. `worktree.baseRef: "fresh"` promises a worktree cut from `origin/<default>`, but the client branches from the **local remote-tracking ref** and only re-fetches when `.git/FETCH_HEAD` is over 24h old — so an agent silently starts from a day-old base, and since any fetch touches FETCH_HEAD, an unrelated `git fetch origin some-branch` re-arms the skip without refreshing anything. Measured both directions against 2.1.237. Two layers: in the primary checkout it refreshes `origin/<default>` in the background (all worktrees of a clone share one object store, so keeping the ref honest fixes the base *at creation*); in a linked worktree it fetches synchronously and `git merge --ff-only`s onto the intended base (the backstop, independent of how the agent was spawned). It can never lose a commit — `--ff-only`, and only when HEAD is already an ancestor of the target; a branch with its own commits, a dirty tree, or a detached HEAD gets an advisory and is left alone. Complements `rebase-guard.sh`: that blocks a stale *push*, this prevents the stale *start*.
- **`worktree-publish.sh`** — `Stop` + `SessionEnd`. Also not a guard, and the counterpart to the one above: freshness fixes how an agent worktree *starts*, this fixes how it *ends*. Closing a finished session was refused with "has commits that are not pushed anywhere", so when an agent in a linked worktree goes idle holding commits on no remote it runs `git push -u origin HEAD:refs/heads/<branch>` and the delete becomes instant. Measured against 2.1.237: `deleteJob` clears a worktree only if `rev-list --max-count=1 HEAD --not --remotes` is empty, **or** the branch's upstream reads exactly `gone` *and* every commit patch-id-matches `origin/<default>` — and an agent worktree is cut `--no-track`, so it has no upstream and that second path can never fire. `force` is on the `dirty` arm and nowhere on the unpushed arm, and no setting or keystroke waives it, so the condition can only be **falsified**. It cannot be done at delete time either: `WorktreeRemove` is dispatched from inside `removeAgentWorktree`, which is reached only *after* the refusal. `-u` is load-bearing — the upstream is what later lets the client's own patch-id path read `gone` and clear merged worktrees unaided. Never forced, never the default branch (rebase-guard tokenizes Bash *tool calls* and cannot see a hook's git, so that rule is re-enforced here), never a stack (`gh stack submit`'s job), and only ever inside a linked worktree that is also under `.claude/worktrees/`. Emits nothing, since a `Stop` hook's stdout is how it would block stopping. Fails open to exactly today's behaviour, with the daily `boom code reap --push` as backstop.
- **`hooks/tests/`** — `run.sh` + `cases.tsv`, a hermetic regression suite for the three guards (throwaway git fixtures in `$TMPDIR`, no network, ~6s). Wired into `lint.yml` and lefthook pre-commit. **Add a case before changing a guard**: these are 200+ lines of security-relevant shell, and shipping them untested is how a `--dry-run` substring in an unrelated commit message came to disable the no-push-to-main rule. Plus **`freshness.sh`**, a separate 14-case suite for `worktree-freshness.sh` — separate because `run.sh` asserts on a `PreToolUse` `permissionDecision` and that hook returns none; its assertion is which commit HEAD lands on. Take its green run with the same suspicion: 9 of its 14 cases are "must not touch this worktree" assertions that a hook doing *nothing at all* passes trivially, so the load-bearing case is the one that requires a fast-forward to actually happen. It accepts a hook path as `$1` so a negative control can point a copy with inverted expectations at the real hook. And **`publish.sh`**, a third 12-case suite for `worktree-publish.sh` — separate again because its assertion is neither a `permissionDecision` nor a HEAD commit but **which refs exist on a bare origin** afterwards. Same suspicion applies and harder: 9 of its 12 cases are "must NOT push", so `publishes_unpushed` is the only case that distinguishes the hook from a no-op, and `never_force` is the one that must never regress. The recorded negative control is inverting `publishes_unpushed` and `skip_default_branch`, which must yield exactly two failures.
...
- **Tool calls gate on a desktop prompt — but "hung until killed" was wrong** (corrected 2026-08-20). Re-measured from a *background* session: `authenticate` returned an account ID promptly, and `list_environments` and `create_environment` both succeeded. So the gate is a real approval prompt, not a hang, and it is answerable whenever someone is at the keyboard. What remains true is the useful half: an approval nobody is present to grant will not self-resolve, so still do not build anything **unattended** on it.

**Caveats worth keeping.** It is beta. Local `.env` mounting is Mac/Linux only. Approvals reset when 1Password locks. 1Password documents setup tabs for Codex and Kiro only — Claude Code is "Other", and there is an open community request for docs; the generic `mcpServers` stdio block is what applies, which is what `claude mcp add` writes. **"Zero Environments exist" was wrong** (corrected 2026-08-20): the account already holds twelve, most of them per-repo (`binfinite-app`, `randsum`, `salvage-union`, `butter`, `opt-fall`, `alxjrvs-github-io`, …). So Environments are an established habit here, not a speculative one, and the server's "earn its place" clock should be read against that rather than against a blank slate — count real invocations (`"name":"mcp__1password__`) before treating it as settled. Per the user-scope rule it should migrate into a specific repo's checked-in `.claude/settings.json` once one project actually owns an Environment, and the per-repo names above are exactly the candidates.
...
| Config | `~/.claude.json` (user scope) | `claude_desktop_config.json` → `op run --env-file` |
| Auth | `headersHelper` → `op-agent header` | `GITHUB_PERSONAL_ACCESS_TOKEN` env, injected in-process |

Desktop can't use the remote server: it authenticates through a registered GitHub App, and Desktop's *Add custom connector* OAuth flow doesn't support that — [GitHub's own `install-claude.md`](https://github.com/github/github-mcp-server/blob/main/docs/installation-guides/install-claude.md) says to run it locally. And Desktop has no `headersHelper`, so it launches the server through **`op run --env-file=~/.config/gh/mcp.env`** — 1Password's own published shape for securing an MCP config — which resolves the `op://` reference in-process and injects it into that one server's environment. **GitHub's documented Desktop config hardcodes the PAT in an `env` block; we deliberately don't** — a `boom verify` check fails if `GITHUB_PERSONAL_ACCESS_TOKEN` ever appears in `claude_desktop_config.json`.

The `gh-mcp-stdio` wrapper this used to need was **deleted on 2026-08-20**: its entire job was to read the PAT and export it, which `op run` does natively. Two consequences worth knowing. Absolute paths for both `op` and the server binary are **required**, not tidiness — Desktop launches as a GUI app with no shell `PATH`, a failure 1Password documents explicitly; `hooks/claude-desktop-mcp.sh` resolves both at sync time. And the auth **tier changed**: the wrapper used the agent service account (keychain, no biometric, worked with Desktop launched at login and 1Password locked), while `op run` uses the desktop integration and will prompt. That is the correct tier — per 1Password's two-tier model Desktop is an *interactive* surface, and the SA is the hands-off agent tier — but the cost is real: with 1Password locked, the server does not start until you unlock.

**If Desktop's GitHub MCP appears dead, check for a pending 1Password prompt BEFORE touching the config.** The failure mode is a *hang*, not an auth error (observed 2026-08-20): `op run` blocks on the approval, so an unanswered prompt looks exactly like a broken binary, a bad path, or malformed JSON — the server simply never finishes its handshake, and Desktop shows no sign that an approval is waiting. Diagnose by running the child by hand — `op run --env-file=~/.config/gh/mcp.env -- <server> --version` — which returns instantly when auth is fine and sits there when it is not.

**The PAT is deliberately NOT duplicated into a 1Password Environment**, even though `op run --environment` is the shape 1Password's *Secure AI Access* page prescribes for MCP configs. Two reasons, one principled and one measured. Principled: `claude-git-pat` is one vault item backing three consumers, and an Environment holding a literal copy would make two sources of truth — rotating the item would silently stop rotating Desktop. Measured (2026-08-20): **`--environment` does not exist in the installed `op` 2.39.0** — it errors `unknown flag: --environment`, because the docs' `v2.33.0-beta.02+` requirement means the *beta* channel, not stable. An Environment named `claude-desktop-mcp` exists holding the *reference* (not the value), ready for the day stable ships the flag; until then it has no consumer. Storing a reference rather than a value is untested — 1Password's docs say Environments "return values exactly as they are entered", which suggests refs are **not** dereferenced, so verify before relying on it.
...
- **`claude` is a shell function in interactive shells** (`zsh/65-claude.zsh`), so `which claude` and what you actually run differ. It runs Claude Code through Claude Code's *own* `ClaudeCode.app` bundle, giving it a stable macOS TCC identity — otherwise every update lands at a new `versions/<ver>` path, which macOS treats as a new app, and all the file-access prompts come back (46 such re-grants in 3 months). It deliberately does **not** replace the installer's `~/.local/bin/claude`, so auto-update and version cleanup stay Anthropic's; it falls through to `command claude` on any doubt. Rationale and the two traps (never `exec`; an alias cannot do it) are in the file and `dot-claude/DECISIONS.md`.
