...
- **`worktree.bgIsolation` defaults to `"none"`**: setting it to `"worktree"` triggers the v2.1.150 commit-lockout — BG sessions are forced into `<repo>/.git/worktrees/<name>/`, but the harness injects per-CWD `denyWithinAllow` entries (`<cwd>/HEAD`, `<cwd>/objects`, `<cwd>/refs`) that block `git add` / `git commit` inside that worktree. Tracked upstream by issues #25896, #17374, #61909, #56331. **Fix B is wired but disabled by default**: a `WorktreeCreate` hook (`dotctl hook worktree-create` in `dot-claude/settings.json`) relocates worktrees to `~/.local/share/cc-worktrees/<repo>/<name>` (outside the cwd-deny radius). Flip `bgIsolation` to `"worktree"` AND test on a *fresh* BG session before relying on it — the hook hasn't been validated end-to-end in production yet (the standard `Agent({isolation: "worktree"})` dispatch path in this CC version is broken, see upstream issue #33045, so it can't be smoke-tested from a parent session).
