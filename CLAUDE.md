...
3. **No gratuitous wrappers.** Call tools natively. Two bash scripts survive, and both do so for the same reason — an external program execs them by path, so there is no native hook to call instead: `op-agent` (a plugin `*_COMMAND` resolver, see Secrets) and `claude-launcher` (macOS execs it as `~/.local/bin/claude`, see hooks/). Git commit signing lives in the `git-signing.ts` hook. The bar for a third is the same: name the external program, and show the native path does not exist.
...
- **`claude-launcher.sh`** — NOT a boom hook: `link`ed to `~/.local/bin/claude`, replacing the symlink the native installer puts there, so Claude Code gets a **stable macOS TCC identity**. macOS keys a file-access grant to the executable's absolute path (`csreq` is NULL on those rows — the code hash is not pinned), and the installer stages every release at its own `~/.local/share/claude/versions/<ver>`, so each update is a brand-new client and every *"wants to access data from other apps"* prompt returns. Measured 2026-08-19: 76 rows in `TCC.db`, 46 of them `kTCCServiceSystemPolicyAppData` — one per update since May. The launcher points Claude Code's own `ClaudeCode.app` bundle (`com.anthropic.claude-code`) at the newest release and execs that, so the identity stops moving. **This is Anthropic's mechanism, not an invention**: Claude Code writes that bundle and already re-execs through it with responsibility disclaimed — but only on the background/PTY-host path, never for the foreground TUI, which is the whole gap. `claude doctor` names a non-installer launcher here and calls it expected when intentional; the cost it names is that automatic version cleanup stops, so the script prunes (newest three). It must be a script and not a symlink to the bundle: Claude Code only refreshes that hardlink when `process.execPath` is under `versions/`, which stops being true the moment a launcher is in front — so a bare symlink would silently freeze the machine on one version forever.

- **`hooks/tests/claude-launcher.sh`** — hermetic regression suite for the launcher (fake `XDG_DATA_HOME` in `$TMPDIR`, stub releases, no network, <1s). **Add a case before changing the launcher**, for the same reason the guards have that rule and a sharper one: this script *is* `claude` on this machine, so its failure mode is not a stale prompt but a machine with no working `claude`. Wired into lefthook pre-commit only — `lint.yml` runs on ubuntu and the launcher is macOS-only by nature (BSD `stat -f %i`), an absence documented in the workflow so nobody re-adds a step that cannot pass. Not to be confused with `dot-claude/hooks/tests/`, the guards' suite.
