...
**Stdio MCP servers get the same treatment via a launch shim** (`spacebase/spacebase-mcp`, registered user-scope by `install/60-claude.sh` alongside the github server). A stdio server's env is static (no `headersHelper` hook — Claude Code expands only `${VAR}` from its own launch env), so a plugin that wants `SPACEBASE_API_KEY` literal would otherwise force a `claude` wrapper or a plaintext export. Instead the shim *is* the server `command`: it reads the keychain SA token inline, `op read`s the key, exports it into **its own** process, and `exec`s the real server — never touching Claude Code's env, a Bash subprocess, the transcript, or OTEL. MCP servers run outside the bash sandbox, so `op` works here (unlike in-box). The gnar `spacebase` plugin bundles its own server that can't resolve the key this way; the user-scope `spacebase` entry shadows it functionally — the bundled server registers no tools (the skills/commands call tools by bare name, which the shim's server provides). The bundled server is silenced via a `deniedMcpServers` entry in `dot-claude/settings.json` so it stops showing as a failed connection in `/doctor` and `/mcp`. **That entry is fragile and needs hand-maintenance**: the denylist only matches by *exact, fully-expanded* `serverCommand` — `${HOME}`/`${CLAUDE_PLUGIN_ROOT}` aren't expanded at match time, wildcards aren't supported, `serverName` can't disambiguate it from the shim (both are named `spacebase`), and the key isn't honored from machine-local `~/.claude.json` (only `settings.json`). So the entry hardcodes the **version-pinned** plugin path (`…/spacebase/<version>/mcp/dist/index.js`) and a literal `/Users/jarvis` — meaning it **silently stops matching (failure reappears) on every plugin version bump**, and it only matches on a machine whose username is `jarvis`. When the failed `plugin:spacebase:spacebase` line returns after an update, bump the version in the `deniedMcpServers` path to the value shown by `claude mcp list`. The key is a manual vault bootstrap (like the Claude Git PAT), not auto-provisioned: `op item create --category "API Credential" --title "Spacebase API Key" --vault claude-agent credential=<sw_token>`.
...
- **dot-claude vs .claude**: `dot-claude/` is the source of truth for **user/global** Claude config (symlinked into `~/.claude/`). The repo-root `.claude/` is this repo's **project-scoped** config: it commits exactly one file — `settings.json`, which carries the sandbox opt-out (`sandbox.enabled:false`, see Sandbox) — while `settings.local.json` is the only `.claude/` path gitignored (machine-local overrides). Don't conflate the two: `dot-claude/` is global, `.claude/settings.json` is this-repo-only project config.
