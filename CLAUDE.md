...
- **`worktree.bgIsolation: "worktree"` works ONLY with Fix B in place** — vanilla CC v2.1.150 with bgIsolation="worktree" triggers the commit-lockout (harness injects per-CWD `denyWithinAllow` on `<cwd>/HEAD`/`objects`/`refs` matching the auto-created `.git/worktrees/<n>/` paths; tracked upstream by #25896 #17374 #61909 #56331). Fix B (`WorktreeCreate` hook → `dotctl hook worktree-create`) relocates worktrees to `~/.local/share/cc-worktrees/<repo>/<n>` (outside the cwd-deny radius), and `~/.local/share/cc-worktrees/**` is in `sandbox.allowWrite` so the relocated working tree itself is writable. Validated end-to-end via `Agent({isolation: "worktree"})` (commit sha was made cleanly inside the relocated worktree). Do NOT remove `~/.local/share/cc-worktrees/**` from allowWrite, do NOT change the WorktreeCreate hook target, and do NOT flip bgIsolation back to "none" without good reason — that surface is load-bearing for parallel-agent isolation now.
