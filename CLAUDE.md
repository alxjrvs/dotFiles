...
- **`pr-review.sh`** — `PostToolUse`: runs the adversarial review locally after `gh pr create` / `git push` / `gh stack submit` and posts it as a PR review + a `claude-review` commit status. Backgrounds itself so it can never block a turn. The `gh stack submit` arm matches neither of the others (Stacks API + in-process push, so no `git push` Bash call), and covers the checked-out layer only. The spawned reviewer is **read-only** (`--allowedTools`: `Read`/`Grep`/`Glob` + read-only `git`/`gh`) — it reads attacker-controlled diff text under `defaultMode: auto` and publishes the result, so it must not be able to execute.
...
- `op-agent status` — reports keychain token presence, then probes the PAT against the GitHub API and prints only a verdict (`on verify`).
...
**Zero calls means "broken or unused" and the two are indistinguishable from usage data alone.** That distinction is why the server came back: its "0 calls across 3,410 transcripts" was read as *unused*, but it had been **broken the whole time** — an unquoted `op://` ref with spaces (below), not a design that failed. Deleting on a usage signal alone removed a working idea. Check `claude mcp list` before concluding a server is dead weight; `boom verify` now fails when any server is down.
