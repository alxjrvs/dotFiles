...
5. **Standard, and agentic-enabled.** 1Password, Git, SSH, `gh`, and the MCP wiring stay stock installs — but wired for agents end-to-end (op MCP, op SSH agent, GitHub MCP, the on-demand auth shims). Secrets resolve through `op` on demand — `op run --env-file` where we control the launch (1Password's own recommendation), or the tool's *native* auth hook fed by `op` where we don't (Claude Code's `headersHelper`, a server's `*_COMMAND` resolver) — never plaintext in git, never exported to the session env, resolved in-process so values don't rest on disk.
...
dot mcp add NAME -- SERVER   # Register a stdio MCP server via `op run --env-file=.env` (refs, off-disk).
...
| `dot mcp` | `./mcp/mcp` | `dot mcp add <name> [--scope S] [--env-file F] [--agent] -- <server>` wraps stock `claude mcp add` so the registered command is 1Password's canonical `op run --env-file=.env -- <server>` — the server's secrets resolve from `op://` references in `.env` at each launch, nothing resolved rests on disk, and the `.mcp.json` entry holds no secret. Default `--scope project` (repo-local committable `.mcp.json`); `--agent` emits a headless recipe that sources the `claude-agent` service-account token from the login keychain inline (no biometric), otherwise `op` uses ambient/desktop auth so the entry stays machine-agnostic. See Secrets management + `templates/mcp/`. |
...
**MCP secrets — one canonical pattern, two op-fed native hooks.** Where we control a server's launch command, secrets follow 1Password's own recommendation ([securing-mcp-servers](https://1password.com/blog/securing-mcp-servers-with-1password-stop-credential-exposure-in-your-agent)): the launch command is **`op run --env-file=.env -- <server>`**, the `.env` holds `op://` *references* (committable, never values), and `op` resolves them into the server process at each launch — nothing resolved rests on disk. Register one with `dot mcp add` (defaults to repo-local `--scope project`); `templates/mcp/` is the starter to copy into a repo. This is the standard for every repo going forward — no per-secret bespoke wiring.

Two surfaces don't expose a launch command we control, so `op run` can't wrap them; on those we feed `op` into the tool's **own native auth hook** instead. These are *not* special cases — they are the only hook the surface offers:

- **HTTP MCP servers** — no child process to wrap, so we use Claude Code's native `headersHelper` hook (below). Resolving via `op run` would instead require wrapping the `claude` launch, leaking the token into every subprocess; the hook keeps it off disk *and* out of the session env.
- **Plugin-bundled stdio servers** — the plugin owns the `command`, so we use its own `*_COMMAND`-style resolver var fed an inline `op read` recipe (spacebase, below).

**Hard line, made structural.** A resolved secret never enters git. The canonical pattern and both hooks resolve in-process (off disk). The lone at-rest fallback — a literal in user-scope `~/.claude.json`, only for an *uncontrolled* stdio server with *no* resolver hook — rests in `$HOME` (mode 0600, never a repo). **Never write a `${VAR}` placeholder into a git-tracked `.mcp.json`/`.env`:** a later `claude mcp add` can expand it and write the resolved token back into the tracked file (Claude Code [#18692](https://github.com/anthropics/claude-code/issues/18692)). The `git-template` pre-commit hook (every repo) and `dot doctor` (this repo + `templates/mcp/`) both fail on a `${VAR}` in a tracked `.mcp.json` and on a resolved-token-shaped value in any tracked `.mcp.json`/`.env`.

**HTTP MCP auth is Claude Code's native `headersHelper` hook, fed by `op`** (`gh/gh-mcp-auth-header`, `render/render-mcp-auth-header`). Each shim sources the keychain token *inline* (`OP_SERVICE_ACCOUNT_TOKEN="$(security find-generic-password -s op-claude-agent -w)" op read …`), confined to that one `op` process — so neither the service-account token nor the resolved secret reaches a Bash subprocess, the transcript, or OTEL spans. The payoff: **no Touch ID prompt and no desktop-app dependency** (works headless/cron). No `claude()` wrapper. When the keychain item is absent the shim falls back to desktop biometric, so nothing breaks pre-bootstrap. Generalize the shim to any HTTP MCP server needing a bearer token; store each such secret in `claude-agent` and point `OP_REF` at `op://claude-agent/...`.

**Plugin-bundled stdio servers resolve their key via the plugin's own resolver command** (for stdio servers *we* install, use the canonical `op run` pattern above instead) — for the gnar `spacebase` plugin's bundled stdio server, no `headersHelper` hook exists (Claude Code expands only `${VAR}` into a stdio plugin server's env from its own launch env). Rather than wrap the server in a launch shim and shadow it (the old approach — a separate user-scope server plus a `deniedMcpServers` entry to hide the failed bundled one), the plugin server itself reads `SPACEBASE_API_KEY_COMMAND` and resolves the key in-process: `dot-claude/settings.json` `env` carries the **recipe** (`OP_SERVICE_ACCOUNT_TOKEN="$(security … -s op-claude-agent -w)" /opt/homebrew/bin/op read 'op://claude-agent/Spacebase API Key/credential'`) — a *non-secret* string, safe in plaintext — and the bundled server runs it, so the SA token and resolved key stay inside that one server process, never touching Claude Code's env, a Bash subprocess, the transcript, or OTEL. `op` is invoked by **absolute path** because the plugin server is spawned at Claude Code startup with a minimal PATH that lacks the homebrew bin dir — a bare `op` fails to resolve there, so the recipe pins `/opt/homebrew/bin/op` (consistent with this repo's existing Apple-Silicon homebrew assumption, e.g. `zsh/30-plugins.zsh`). When the keychain item is absent `op read` falls back to desktop biometric. This is the native fix tracked in `TheGnarCo/agent-skills#368` — it retired the `spacebase/spacebase-mcp` shim, its symlink + user-scope registration, and the fragile version-pinned denylist. **Requires a plugin version that reads `SPACEBASE_API_KEY_COMMAND`** (passing it through in its `mcpServers` env); on an older plugin the bundled server fails until upgraded. The key is a manual vault bootstrap (like the Claude Git PAT), not auto-provisioned: `op item create --category "API Credential" --title "Spacebase API Key" --vault claude-agent credential=<sw_token>`.
...
- **Never add a token to a config file as plaintext.** If `.npmrc`-shape tools need credentials, use `op://` refs + `op run --`. The lone resting exception is a literal in user-scope `~/.claude.json` (`$HOME`, mode 0600, never a repo), and only for an uncontrolled stdio MCP server with no resolver hook — not for anything tracked.
- **MCP servers we install** follow the canonical pattern: `op run --env-file=.env -- <server>` with `op://` references in `.env` (`dot mcp add`). HTTP servers use the `headersHelper`→`op read` hook; plugin-bundled servers use their `*_COMMAND` resolver. See *MCP secrets* above.
- **Never write a `${VAR}` placeholder into a git-tracked `.mcp.json`/`.env`** — `claude mcp add` can expand it and write the resolved token back into the tracked file (#18692). Use `op run --env-file` (refs in `.env`) or a `headersHelper`.
...
- `gitleaks` runs from the pre-commit hook template (`git-template/hooks/pre-commit`, copied into new repos via `init.templateDir`); the same hook also fails staged `.mcp.json`/`.env` that carry a `${VAR}` placeholder or a resolved-token literal instead of `op://` references. Both are backstops, not the policy.
