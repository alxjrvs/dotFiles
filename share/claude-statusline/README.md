...
- **Line 6** — session cost, $/h burn rate, and (when other sessions contributed) today's cross-session total.
...
- Daily cost total is tracked under `~/.claude/state/cost/<date>/` (one file per session); delete that tree any time.
