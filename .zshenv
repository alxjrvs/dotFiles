# Dotfiles root — used by the `dots` alias. Self-locating: resolves this
# (symlinked) file back to its source, so any clone location works with no
# hardcoded path. Exported so every shell context inherits it.
#
...
# shells (the env outlives the move) and breaks `dots`.
