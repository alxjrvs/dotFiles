...
# re-push).
#
# Wired agent-side via home/dot_claude/settings.json `hooks.PreToolUse` (matcher
...
#
# Portability: bash 3.2 (`/bin/bash`) — a hook cannot assume its PATH. No arrays;
# state is carried in scalars and positional params, which is also why the
# segment list is walked twice rather than accumulated.
set -u
...
# `git -c alias.X=push X origin main` reaches main: the global-flag loop below
# consumes `-c alias.yolo=push` as an ordinary value-taking flag and then reads
# the subcommand as `yolo`, so the command is never recognised as a push at all
# and every refspec check downstream is skipped.
#
...
# by every call to catch a spelling nobody uses by accident.
...
# than being refused wholesale. See _expand_interpreters in guard-lib.sh for why
# the pipe case needs the whole command and why it cannot reach inside a commit
# message.
...
while IFS= read -r seg; do
  ...
  shift
  # `git -C <path>` names the repo THIS invocation acts on. It is NOT `cd`: `cd` persists across
  # `&&` segments, so honouring it for the whole command is right, while `-C` binds to the single
  # command carrying it. Letting an EARLIER segment's `-C` retarget a later push is how
  # `git -C /tmp status && git push origin main` walks past — work_dir becomes /tmp, the
  # is-inside-work-tree probe fails there, and the guard FAILS OPEN on a push to the default branch.
  #
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      # Discarding -C entirely was the ORIGINAL bug, and it is still a bug: it judges every
      # cross-repo push against the SESSION's cwd repo — a legitimate push blocked by an unrelated
      # checkout's staleness, and a push to another repo's default branch judged against the wrong
      # default. It has to be read; it just must not leak across segments.
      -C)
        shift
        if [ $# -gt 0 ]; then
          d=$(_unquote "$1")
...
# EVERY push segment is judged, not just the last. Overwriting a single
# `push_seg` allowed `git push origin main && git push origin main --dry-run` on
# the strength of the trailing dry run.
if [ "$is_push" = 1 ]; then
  while IFS= read -r seg; do
    [ -n "$seg" ] || continue
    set -f
    ...
    # Walk the push arguments as TOKENS. String-slicing (`${push_seg#*push}`)
    # strips through the first literal "push" anywhere in the segment, so
    # `git -c push.default=current push` never looks like a bare push.
    dry=0
    skipnext=0
    remote_seen=0
    refspecs=''
...
# front of every push and every `gh pr create`. Unbounded, a hung DNS lookup or
# an unreachable remote stalls the publish path for as long as git waits, inside
# a PreToolUse hook, with the agent simply blocked.
#
# A timed-out fetch is not a failure: the comparison below then runs against the
# last-known `origin/<branch>`, and a stale ref can only make this guard MISS a
# behind-target push, never invent one — the correct direction for a guard that
# fails open everywhere else. `timeout` is not in the macOS base system; absent,
# the fetch runs unbounded rather than being skipped.
#
# The fetch is skipped when the remote-tracking ref is already fresh, collapsing
# a burst of pushes into one round trip; 120 s still catches a teammate landing
# on the target between two pushes of your own.
# FETCH_HEAD, not the remote-tracking ref: it is written by every fetch and it
# exists as a real file, where `refs/remotes/...` lives in the COMMON git dir
# rather than a worktree's own gitdir and may not be loose once refs are packed.
# BOTH git dirs, and the per-worktree one FIRST: `git -C <worktree> fetch` writes
# .git/worktrees/<name>/FETCH_HEAD and leaves the common one untouched, so
# checking only the common dir makes this cache never hit from inside a
# worktree — which is where every agent session runs.
fresh=0
