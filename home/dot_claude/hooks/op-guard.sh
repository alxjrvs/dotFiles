...
#
# `security` and `git` are here because each has ONE subcommand that prints a
# live credential and neither contains an `op` token. Each is anchored on that
# SUBCOMMAND, not on the program name, and that is load-bearing rather than
# tidy: waking on the bare word `git` drags every ordinary
...
printf '%s' "$cmd" | grep -qE '(^|[^A-Za-z0-9_.-])(op(-agent)?([^A-Za-z0-9_-]|$)|security([^A-Za-z0-9_-]|$).*find-(generic|internet)-password|security([^A-Za-z0-9_-]|$).*find-(certificate|identity)|git([^A-Za-z0-9_-]|$).*credential)' || allow
