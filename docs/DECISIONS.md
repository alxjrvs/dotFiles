...
in `home/dot_claude/hooks/tests/cases.tsv`, not here — the test is the record.
...
this file, `CLAUDE.md` and `home/dot_claude/CLAUDE.md` for the flag, for "agent teams" and for
...
in `home/dot_claude/agents/` all serve the ordinary `Task` tool rather than teams, and all three are
live — `guard-tester` runs the gate suites and `drift-triage` reads `scripts/verify.sh`. An
...
`drift-triage` and `guard-tester` stay here. One reads `scripts/verify.sh` output; the other runs
...
where chezmoi already links `home/dot_claude/skills/`, and again under the plugin's own
...
binary mise resolves. These are what `chezmoi apply` itself touches, which is the single command
...
repo. The question was whether the protection follows the link back to `home/dot_claude/…`. **It
...
| **denied WITHIN the allowed repo** | `settings.json`, `home/dot_claude/hooks`, `home/dot_claude/skills`, `.credentials.json` |
| `home/dot_claude/CLAUDE.md` | allowed |
...
An audit costed `home/dot_claude/hooks/tests/cases.tsv` at −230 lines: 347 of its 664 lines are
...
**And they cost nothing.** `cases.tsv` is under `.chezmoiignore`, so it is never symlinked to a
...
## 2026-09-01 — a plugin was considered and declined; the dotfiles engine is the delivery mechanism
...
**The engine already delivers all of it, and the two paths are mutually exclusive rather than
additive.** chezmoi symlinks `home/dot_claude/{hooks,skills,agents,rules}` into `~/.claude/`, and
every hook handler in `settings.json` is an absolute `~/.claude/hooks/*.sh` path. A plugin
...
It is also the exact pattern this audit condemned elsewhere — the two hand-maintained CI/lefthook
rosters that had already drifted.
...
`home/dot_claude/settings.json` during this work and then reverted, because *the credential scrub was
...
tracked and symlinked into `~/.claude/`, so the app's write lands in the checkout and shows up
in `git status` forever after. Removing the key buys one clean commit and then a permanently
dirty tree the next time the app rewrites it. A key you cannot keep out is not a key you can
delete.
...
`/config` confirmation** — check the file.
